                ...
                if len(lst_string) != 3:
                    try:
                        ...
                        with open(self.registration_bad_log, 'a', encoding='utf8') as code:
                            code.write(
                                f"{number_string}    Поймано исключение  {v_error} --- ValueError ==> {len(lst_string)} Аргументов в массиве\n")
                        print(
                            f"{number_string}    Поймано исключение  {v_error} --- ValueError ==> {len(lst_string)} Аргументов в массиве\n")
                    ...
                    try:
                        ...
                        with open(self.registration_bad_log, 'a', encoding='utf8') as code:
                            code.write(
                                f"{number_string}    Поймано исключение  {v_error} --- ValueError ==> {lst_string[-1]} Это не возраст\n")
                        print(
                            f"{number_string}    Поймано исключение  {v_error} --- ValueError ==> {lst_string[-1]} Это не возраст\n")
                    ...
                    try:
                        ...
                        with open(self.registration_bad_log, 'a', encoding='utf8') as code:
                            code.write(
                                f"{number_string}    Поймано исключение  {v_error} --- ValueError ==> {int(lst_string[2])} Указанный возвраст не воходит в кретерий\n")
                        print(
                            f"{number_string}    Поймано исключение  {v_error} --- ValueError ==> {int(lst_string[2])} Указанный возвраст не воходит в кретерий\n")
                    ...
                    try:
                        ...
                        with open(self.registration_bad_log, 'a', encoding='utf8') as code:
                            code.write(
                                f"{number_string}    Поймано исключение {NotEmailError} --- NotEmailError ==> {lst_string[1]} Поле емейл НЕ содержит @ и .(точку)\n")
                        print(
                            f"{number_string}    Поймано исключение {NotEmailError} --- NotEmailError ==> {lst_string[1]} Поле емейл НЕ содержит @ и .(точку)\n")
                    ...
                    try:
                        ...
                        with open(self.registration_bad_log, 'a', encoding='utf8') as code:
                            code.write(
                                f"{number_string}    Поймано исключение {NotNameError} --- NotNameError ==> {lst_string[0]} Поле имени содержит НЕ только буквы\n")
                        print(
                            f"{number_string}    Поймано исключение {NotNameError} --- NotNameError ==> {lst_string[0]} Поле имени содержит НЕ только буквы\n")
                number_string += 1
